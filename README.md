...
## Daily Safe Shorts Automation

This project can also run as a conservative daily automation system that uses
YouTube trends for topic discovery, checks source permissions, creates
transformative Shorts, and uploads through the official YouTube Data API.

What it does:

- Finds trending videos and topics with the YouTube Data API.
- Processes only allowed sources: your own/whitelisted channels, explicitly
  allowed URLs, Creative Commons sources when enabled, or local files you
  provide.
- Adds original commentary as an overlay before upload.
- Uploads private by default for review.
- Logs rejected videos and upload decisions in SQLite.

What it intentionally does not do:

- It does not scrape YouTube trends when API quota fails.
- It does not blindly download trending creators' videos and reupload clips.
- It does not publish publicly unless `AUTO_PUBLISH=true` and all compliance
  checks pass with low risk.

### Automation setup

Install the base automation dependencies:

```bash
pip install -r requirements.txt
```

For local generation, also install:
...
YOUTUBE_OAUTH_TOKEN_FILE=token.json
AUTO_PUBLISH=false
DEFAULT_UPLOAD_PRIVACY=private
DAILY_MAX_UPLOADS=1
DAILY_MAX_SOURCE_VIDEOS=10
MIN_TREND_SCORE=70
SHORT_MAX_DURATION_SECONDS=58
ALLOWED_SOURCE_POLICY=strict
...
Edit `config/automation.yaml` to configure allowed channels, blocked channels,
allowed URLs, topic keywords, excluded keywords, default tags, scheduling, and
compliance rules.

Run OAuth once before uploading:

```bash
python -m shorts_generator.upload.auth
# or
python automation.py auth-youtube
```
...
```bash
python automation.py trends --region US --max-results 10
```

Recommended first run:

```bash
AUTO_PUBLISH=false DEFAULT_UPLOAD_PRIVACY=private DAILY_MAX_UPLOADS=1 \
python automation.py run-once --region US --mode local --max-uploads 1 --dry-run
```

Run one live pass:

```bash
python automation.py run-once --region US --mode local --max-uploads 1
```

Upload an existing rendered Short privately:

```bash
python automation.py upload ./output/short_01.mp4 --title "My Short #Shorts" --privacy private
```

Report recent logs:

```bash
python automation.py report --last 7
```
...
- Add repository secrets for `YOUTUBE_API_KEY` and `MUAPI_API_KEY`.
- The included `.github/workflows/daily-shorts.yml` runs daily and supports
  manual dispatch.
- OAuth tokens are intentionally not committed.
- Logs are uploaded as workflow artifacts.

Local cron:

```cron
0 10 * * * cd /path/to/project && ./venv/bin/python automation.py run-once --region US --mode local --max-uploads 1 >> logs/daily.log 2>&1
```

### Copyright and reused-content safety notes

Use trending data for ideas and topic discovery. Do not treat a trending public
video as permission to reuse the video. Keep `AUTO_PUBLISH=false`,
`DEFAULT_UPLOAD_PRIVACY=private`, and `DAILY_MAX_UPLOADS=1` until your
whitelists, licenses, and review process are proven. If the system is uncertain
about source rights or transformation quality, it rejects the source or avoids
public publishing.
