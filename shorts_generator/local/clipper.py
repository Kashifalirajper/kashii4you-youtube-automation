...
def _reframe_vertical(in_path: str, out_path: str, aspect_ratio: str) -> str:
    ...
    if not hasattr(cv2, "CascadeClassifier"):
        return _reframe_center_ffmpeg(in_path, out_path, aspect_ratio)
    ...
    subprocess.run(cmd, check=True)
    _remove_temp_file(silent_path)
    return out_path


def _reframe_center_ffmpeg(in_path: str, out_path: str, aspect_ratio: str) -> str:
    """Fallback crop for OpenCV builds without Haar cascades."""
    target_ratio = _ratio(aspect_ratio)
    if target_ratio <= 1:
        out_w = 720
...
def _remove_temp_file(path: str) -> None:
    for attempt in range(5):
        try:
            ...
            if attempt == 4:
                print(f"[clip/local] warning: could not remove temporary file: {path}", flush=True)
                return
            time.sleep(0.25)
