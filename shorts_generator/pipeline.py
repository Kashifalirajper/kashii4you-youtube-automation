...
from .highlights import call_muapi_llm, get_highlights, get_highlights_heuristic
...
def _run_local(
    ...
    if LLM_PROVIDER == "heuristic":
        print("[highlights/local] using free heuristic selector (no LLM/API)", flush=True)
        highlights_result = get_highlights_heuristic(transcript, num_clips=num_clips)
    else:
